...
(function () {
	var jdchart = angular.module('xdata.jdchart', ['xdata.jdchart.service']);
	
	jdchart.directive('jdchart', ['jdchartService', '$timeout', jdchartDirective]);

	function jdchartDirective(jdchartService, $timeout) {
		...
		var getMergedOptions = function (scope, element, config) {
			...
			if (config.options) {
				mergedOptions = jdchartService.deepExtend(defaultOptions, config.options);
			} else {
			...
			angular.forEach(axisNames, function (axisName) {
				if (angular.isDefined(config[axisName])) {
					mergedOptions[axisName] = jdchartService.deepExtend(mergedOptions[axisName] || {}, config[axisName]);

					if (angular.isDefined(config[axisName].currentMin) ||
						angular.isDefined(config[axisName].currentMax)) {

						jdchartService.prependMethod(mergedOptions.chart.events, 'selection', function (e) {
							var thisChart = this;
							if (e[axisName]) {
						...
						jdchartService.prependMethod(mergedOptions.chart.events, 'addSeries', function (e) {
							scope.config[axisName].currentMin = this[axisName][0].min || scope.config[axisName].currentMin;
							scope.config[axisName].currentMax = this[axisName][0].max || scope.config[axisName].currentMax;
						});
						jdchartService.prependMethod(mergedOptions[axisName].events, 'setExtremes', function (e) {
		...
		var chartOptionsWithoutEasyOptions = function (options) {
			return angular.extend(
				jdchartService.deepExtend({}, options), {
				data : null,
				visible : null
			});
		};
		...
		function linkWithHighcharts(Highcharts, scope, element, attrs) {
			...
			var processSeries = function (series, seriesOld) {
				...
				for (i = chart.series.length - 1; i >= 0; i--) {
					...
					if (s.options.id !== 'highcharts-navigator-series' && jdchartService.indexOf(ids, s.options.id) < 0) {
						s.remove(false);
					}
				}
		...
		function link(scope, element, attrs) {
			...
			jdchartService.getHighcharts().then(highchartsCb);
		}
