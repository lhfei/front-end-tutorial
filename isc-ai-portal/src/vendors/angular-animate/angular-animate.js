...
// "OR" statement then it would not be enough to catch that.
	...
	const $AnimateCssProvider = ['$animateProvider', /** @this */ function ($animateProvider) {
		const gcsLookup = createLocalCacheLookup()
		const gcsStaggerLookup = createLocalCacheLookup()
      ...
      // then staggering is possible
        ...
        // during this looping then it will consider new requests
				...
				function computeTimings (node, className, cacheKey) {
					const timings = computeCachedCssStyles(node, className, cacheKey, DETECT_CSS_PROPERTIES)
      ...
      // there actually is a detected transition or keyframe animation
					...
					const preparationClasses = [structuralClassName, addRemoveClassName].join(' ').trim()
        ...
        // if the preparation function fails then the promise is not setup
					...
					function applyBlocking (duration) {
						...
						if (flags.blockKeyframeAnimation) {
							blockKeyframeAnimations(node, Boolean(duration))
						}
					}
...
//  by the time...
    ...
    // IE9-11 has no method "contains" in SVG element and in Node.prototype. Bug #10259.
      ...
      // eslint-disable-next-line no-bitwise
					return this === arg || Boolean(this.compareDocumentPosition(arg) & 16)
				}
      ...
      //  (element, bool) - element setter<F37>
					enabled (element, bool) {
						...
						if (argCount === 0) {
          // () - Global getter
							bool = Boolean(animationsEnabled)
						}
						else {
							...
							if (!hasElement) {
            // (bool) - Global setter
								bool = animationsEnabled = Boolean(element)
							}
							else {
								const node = getDomNode(element)
      ...
      // to the user), because browsers slow down or do not flush calls to requestAnimationFrame
					let skipAnimations = !animationsEnabled || documentHidden || disabledElementsLookup.get(node)
					const existingAnimation = (!skipAnimations && activeAnimationsLookup.get(node)) || {}
					const hasExistingAnimation = Boolean(existingAnimation.state)
...
/* exported $$AnimationProvider */
    ...
    // TODO(matsko): document the signature in a better way
      ...
      // for now they just stop the animation from starting
					...
					const tempClasses = options.tempClasses
					if (tempClasses) {
						classes += ' ' + tempClasses
 ...
 * </example>
 */
	const ngAnimateSwapDirective = ['$animate', '$rootScope', function ($animate, $rootScope) {
		return {
			restrict: 'A',
			transclude: 'element',
			terminal: true,
